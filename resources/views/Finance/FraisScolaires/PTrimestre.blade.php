<div class="card-body pb-0">
    <a class="btn btn-light btn-block mb-2"><b>1<sup>èr</sup> Trimestre / {{$moda}}</b></a>
</div>
@foreach($FraisFixePourEleve as $modalite)
    @if(! $modalite->montant_franc == null or ! $modalite->montant_dollar == null )
    @if($FraisPremierTPayeParEleveEnFranc > $modalite->montant_franc or $FraisPremierTPayeParEleveEnDollar >
    $modalite->montant_dollar)
        ...
        <p class="text-center"><i>Vous venez de payer pour le premier trimestre qui est déjà soldé !</i></p>
    @else
        ...
        <div class="card-box min-height-200px pd-20 mb-20" data-bgcolor="#265ed7">
            <div class="d-flex justify-content-between pb-20 text-white">
                ...
                <div class="font-14 text-right">
                    ...
                    <div class="font-12">1<sup>èr</sup> Trimestre / {{$moda}}</div>
                </div>
            </div>
        ...
        <div class="card-box min-height-200px pd-20 mb-20" data-bgcolor="#00b489">
            <div class="d-flex justify-content-between pb-20 text-white">
                ...
                <div class="font-14 text-right">
                    ...
                    <div><i class="icon-copy ion-arrow-up-c"></i> {{sprintf('%.0f', pourcentage($FraisPremierTPayeParEleveEnFranc, $fixe->montant_franc))}}%</div>
                    @else
                    <div><i class="icon-copy ion-arrow-up-c"></i> {{sprintf('%.0f', pourcentage($FraisPremierTPayeParEleveEnDollar, $fixe->montant_dollar))}}%</div>
                    @endif
                    @endforeach
            ...
            <div class="d-flex justify-content-between align-items-end">
                <div class="text-white">
                    ...
                    @foreach($FraisFixePourEleve as $fixe)
                    @if(!$fixe->montant_franc == '')
                    <div class="font-30 weight-500">CDF {{number_format($FraisPremierTPayeParEleveEnFranc)}}</div>
                    @else
                    <div class="font-30 weight-500">$ {{number_format($FraisPremierTPayeParEleveEnDollar)}}</div>
                    @endif
                    @endforeach
                </div>
        ...
        <div class="card-box min-height-200px pd-20 mb-20" data-bgcolor="#455a64">
            ...
            <div class="d-flex justify-content-between align-items-end">
                <div class="text-white">
                    ...
                    @foreach($FraisFixePourEleve as $fixe)
                    @if(!$fixe->montant_franc == '')
                    <div class="font-30 weight-500">CDF {{number_format($fixe->montant_franc - $FraisPremierTPayeParEleveEnFranc)}}
                    </div>
                    @else
                    <div class="font-30 weight-500">$ {{number_format($fixe->montant_dollar - $FraisPremierTPayeParEleveEnDollar)}}
                    </div>
                    @endif
                    @endforeach
                </div>
