...
@endif
 ...
 ///////////////modification de recu de frais////////////////////////////
 $(document).on('click', '.modi_re', function (e) {
            
            var notyConfirm = new Noty({
                text: '<h6 class="mb-3">Modifier le montant</h6><label>Enter comment (optional)</label> <input type="text"  class="form-control montant" placeholder="Inserer le montant">',
                timeout: false,
                modal: true,
                layout: 'center',
                closeWith: 'button',
                type: 'confirm',
                buttons: [
                    Noty.button('Annuler', 'btn btn-light', function () {
                        notyConfirm.close();
                    }),

                    Noty.button('Confirmer <i class="icon-paperplane ml-2"></i>', 'btn bg-blue ml-1', function () {
                            var id = $('.modi_rec').data('id');
                            var data = {
                                'montant': $('.montant').val(),
                            }
                            $.ajaxSetup({
